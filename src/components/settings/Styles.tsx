...
})<StyledCardProps>(({ selected, theme }: any) => ({
  height: 72,
  display: "flex",
  alignItems: "center",
...
interface MaskControlPros {
  value: string;
}

export function MaskControl({ value }: MaskControlPros) {
  return (
    <FormControlLabel
      label=""
      value={value}
...
export const LayoutIcon = ({ layout }: LayoutIconProps) => {
  const WIDTH = 16;
  const HEIGHT = 10;
  const RADIUS = 0.5;
  const SPACING = 0.5;
...
interface StyledCircleColorProps {
  selected?: boolean;
  color: string;
}
